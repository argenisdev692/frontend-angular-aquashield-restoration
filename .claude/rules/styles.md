...
# Styling Rules — Angular 21 with PrimeNG v21 Theming

> **Authority**: SINGLE SOURCE OF TRUTH for all styling decisions in this project.
> **Scope**: PrimeNG **styled mode** (`@primeuix/themes` + Aura preset + `cssLayer`), styles.css design tokens, Tailwind v4, Pass Through API, dark mode.

## Core Principles

1. **Token-First Design**: All colors, spacing, and typography come from CSS custom properties in `styles.css`
2. **PrimeNG Styled Theming**: Use the `@primeuix/themes` preset engine; customize tokens with `definePreset`, keep `cssLayer` enabled so Tailwind v4 overrides cleanly. (Legacy `unstyled` mode is NOT used.)
3. **Pass Through API**: Attach `styles.css` classes / Tailwind utilities to PrimeNG internals via `pt` (augments the theme, does not replace it)
4. **Dark-First**: Default dark theme (`:root`) with light override via `[data-theme="light"]`; PrimeNG dark tokens trigger on the `.dark` class
...
### App Config Setup (actual)
```typescript
// src/app/app.config.ts
import { providePrimeNG } from 'primeng/config';
import Aura from '@primeuix/themes/aura';   // ✅ preset from @primeuix/themes — NOT 'primeng/themes'
...
providePrimeNG({
  theme: {                                   // ✅ styled mode — no `unstyled: true`
    ...
    options: {
      ...
      cssLayer: { name: 'primeng', order: 'primeng' } // ✅ lets Tailwind v4 override PrimeNG
    }
  }
})
```

> Customize brand tokens with `definePreset(Aura, { semantic: { primary: { 500: '#00B5E2' } } })` and keep them in sync with `--accent-primary`. Apply `styles.css` classes to components with the `pt` Pass Through API below.
...
✅ ALWAYS use PrimeNG styled theming (`@primeuix/themes`) with `cssLayer` + Pass Through
✅ ALWAYS map styles.css classes to PrimeNG components via `pt`
✅ ALWAYS use class bindings instead of `ngClass`
✅ ALWAYS use style bindings instead of `ngStyle`
✅ ALWAYS test dark (`.dark`) AND light (`[data-theme="light"]`) modes

### DON'Ts
❌ NEVER use hardcoded hex colors
❌ NEVER use raw Tailwind color scale
❌ NEVER disable `cssLayer` or fork the whole PrimeNG theme (customize via `definePreset`)
...
```css
/* Dark is the default (:root). The .dark class is added by ThemeService and also
   triggers PrimeNG dark tokens via darkModeSelector: '.dark'. */
:root,
.dark {
...
- [ ] Use PrimeNG styled theming (`@primeuix/themes`) with `cssLayer`
- [ ] Map tokens/classes via Pass Through API
- [ ] Test dark mode (`.dark`)
- [ ] Test light mode (`[data-theme="light"]`)
